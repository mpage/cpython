...
static inline void _PyObject_SET_GC_BITS(PyObject *op, uint8_t bits) {
    _Py_atomic_or_uint8_relaxed(&op->ob_gc_bits, bits);
}

static inline int _PyObject_HAS_GC_BITS(PyObject *op, uint8_t bits) {
    return (_Py_atomic_load_uint8_relaxed(&op->ob_gc_bits) & bits) != 0;
}

static inline void _PyObject_CLEAR_GC_BITS(PyObject *op, uint8_t bits) {
